...
`"use server"` module:
- `buildPrompt({ name, phone, note, title, location })` — **pure function**,
  returns the user-message string sent to the model. Instructs: short, casual,
  first-person, name the actual item, ask if it's still available, offer
  same-day pickup, include the phone, sound like a real person — not salesy,
  not templated, not AI-generated.
- `draftPickupMessage(listingId)` — loads listing + owner (reuse / extend
  `getListing`), calls Anthropic Messages API via `fetch` with
  `claude-haiku-4-5`, returns the generated text. Throws a friendly error if
...
logic:
- `buildPrompt()` includes the title, name, phone, and note when provided, and
  omits the phone line cleanly when phone is empty.
